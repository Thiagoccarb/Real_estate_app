...
class AddAddressServiceTests(IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.service = AddAddressService(self.address_repository, self.city_repository)
    ...
    async def test_add_address(self):
        ...
        self.city_repository.add.return_value = City(id=1, name="test", state="AB")

        self.address_repository.add = CoroutineMock()
        self.address_repository.add.return_value = Address(
            id=1, street_name="test", city_id=1, cep="11111-111"
        )
            ...
            AddressWithCity(
                **{
                    **Address(
                    ...
                    "city": {
                        **City(id=1, name="test", state="AB").dict(exclude={"id"})
                    },
                }
            ),
        )
        ...
        self.city_repository.find_by.return_value = City(id=1, name="test", state="AB")
